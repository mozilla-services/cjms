...
async fn aic_create(aic_id: Uuid, db_pool: &PgPool) -> Uuid {
    sqlx::query_as!(
        AIC,
...
pub async fn create(_data: web::Json<AICRequest>, pool: web::Data<PgPool>) -> HttpResponse {
    ...
    let id = aic_create(aic_id, pool.as_ref()).await;
...
pub async fn update(req: HttpRequest, _data: web::Json<AICRequest>) -> impl Responder {
